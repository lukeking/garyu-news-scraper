...
- [X] T029 [US3] 依 `quickstart.md` 驗收 FR-016／FR-017，**必須在圖片覆蓋率 0% 的舊週實測一次**（SC-005）
...
**T029 零圖片退化**：使用者判斷不需特地去 W28——W29 本身就有 67% 的文章沒有圖，
混合情境下版面無違和。這比全空的舊週更接近真實使用，接受。

**T029 帶出的缺口（已記入 `specs/BACKLOG.md`）**：US3 驗收情境 2 要求
「具備圖片者在**檢視列表**時視覺重量較高」，但縮圖被放進預設隱藏的 `tr-detail`，
因此掃描視圖裡有沒有圖毫無差別。使用者原話：「沒辦法從收合狀態一眼看出哪一則是有圖的。」
US3 的星號標記那半有做到，圖片這半沒有。補法需要動列模型，屬新題。

**RSS 不受影響**：feed 以 Media RSS 輸出圖片，使用者於 Feedly 實測確認有圖項目正常顯示。
